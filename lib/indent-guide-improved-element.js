...
const styleGuide = function(element, point, length, stack, active, editor, basePixelPos, lineHeightPixel, baseScreenRow, scrollTop, scrollLeft) {
  ...
  const left = ((point.column+0) * indentSize * editor.getDefaultCharWidth()) - scrollLeft;
  const top = (basePixelPos + (lineHeightPixel * (row - baseScreenRow))) - scrollTop;
...
const createElementsForGuides = function(editorElement, fns) {
  const itemParent = editorElement.querySelector('.scroll-view');
  ...
  __range__(0, createNum, false).forEach(function(i) {

...
var toG = function(indents, begin, depth, cursorRows) {
  let ptr = begin;
  let isActive = false;
  let isStack = false;
...
const getGuides = function(visibleFrom, visibleTo, lastRow, cursorRows, getIndentFn) {
  const visibleLast = Math.min(visibleTo, lastRow);
  const visibleIndents = __range__(visibleFrom, visibleLast, true).map(getIndentFn);
...
const statesInvisible = function(cursorRows, start, getIndentFn, lastRow, isAbove) {
  ...
  for (let i of Array.from((isAbove ? __range__(start, 0, true) : __range__(start, lastRow, true)))) {
    const ind = getIndentFn(i);
    ...
    if (cursors[0] === i) {
      ...
      if (stack.length === 0) { stack = __range__(0, minIndent - 1, true); }
    }
  }
  return {
...
var statesAboveVisible = (cursorRows, start, getIndentFn, lastRow) => statesInvisible(cursorRows, start, getIndentFn, lastRow, true);

var statesBelowVisible = (cursorRows, start, getIndentFn, lastRow) => statesInvisible(cursorRows, start, getIndentFn, lastRow, false);

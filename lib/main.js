...
// Cache for editor state to avoid redundant calculations
const editorStateCache = new WeakMap()
...
module.exports = {
  ...
  deactivate() {
    ...
    editorStateCache.clear?.() // Clear cache on deactivate if supported
  },
  ...
  updateGuide(editor, editorElement) {
    ...
    const cacheKey = `${visibleRange[0]}-${visibleRange[1]}-${cursorRows.join(',')}`
    const cachedState = editorStateCache.get(editor)
    if (cachedState === cacheKey) { return }
    editorStateCache.set(editor, cacheKey)
